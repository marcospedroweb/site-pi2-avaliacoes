...
class Titulo extends Model
{
  ...
  public function criar(array $dados): array
  {
    ...
    $sql = "INSERT INTO titulo
              (nome, sinopse, capa, posX, posY, nota, categoria, temporadas, episodios, duracao, temporadaUnica) 
            VALUES 
              (:nome, :sinopse, :capa, :posX, :posY, :nota, :categoria, :temporadas, :episodios, :duracao, :temporadaUnica)";
  ...
  public function listar(array $dados): array
  {
    ...
    $orderBy = $dados['ordenado_por'] ?? '';
    $pagina_atual = $dados['pagina_atual'] ?? '';
    $proximas_linhas = 16;

    //Verificando se a variavel de paginação é valida
    if ($pagina_atual === '' || !(is_numeric($pagina_atual)) || $pagina_atual === 0 || $pagina_atual === 1 || $pagina_atual < 0)
      $pagina_atual = 0; //Se não for, adiciona a pagina padrão
    else if ($pagina_atual !== 0 && $pagina_atual !== 1)
      $pagina_atual = ($pagina_atual - 1) * 16;
    ...
    //Sql para o select
    $sql = "SELECT
              t.id, t.nome, sinopse, capa, posX, posY, nota, c.tipo, temporadas, episodios, duracao,temporadaUnica 
            FROM titulo as t INNER JOIN categoria as c
            ON t.categoria = c.id
            WHERE c.tipo like '%:categoria%' and t.nome like '%:titulo_nome%'
            ORDER BY :ordernado_por
            OFFSET :pagina_atual ROWS
            FETCH NEXT :proximas_linhas ROWS ONLY";

    $select = $this->prepare($sql);
    $resultado = $select->execute([
      ':categoria' => $categoria,
      ':titulo_nome' => $titulo_nome,
      ':ordenado_por' => $orderBy,
      ':pagina_atual' => $pagina_atual,
      ':proximas_linhas' => $proximas_linhas
    ]);
