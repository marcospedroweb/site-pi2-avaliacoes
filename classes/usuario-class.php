...
class Usuario {
  ...
  function criarConta($postCriarConta){
    ...
    $sql = "SELECT id, email
            FROM usuario
            WHERE email = ?";
    $registrosUsuario = array($email);
    //Executando o select
    $select = sqlsrv_query($this->conn, $sql, $registrosUsuario);
    if($select){
      $registro = sqlsrv_fetch_array( $select, SQLSRV_FETCH_ASSOC);
      if($registro['id'])
    ...
    $sql = "SELECT id 
            FROM usuario 
            WHERE email = ?";
    $select = sqlsrv_query($this->conn, $sql, array($email));
    $registro = sqlsrv_fetch_array( $select, SQLSRV_FETCH_ASSOC);
  ...
  function login($postLogin){
    ...
    $sql = "SELECT 
            ...
            FROM usuario 
            WHERE email = ?";
    $select = sqlsrv_query($this->conn, $sql, array($email));//Executa a consulta, comparando com o email passado
    $registro = sqlsrv_fetch_array( $select, SQLSRV_FETCH_ASSOC);//Retorna o usuario compativel com o email
    ...
    $_SESSION['posX'] = $registro['posX'];
    $_SESSION['posY'] = $registro['posY'];
    $_SESSION['zoom'] = $registro['zoom'];
  ...
  function mostrarConta($idUsuario){
    ...
    if($select)
      $registroUsuario = sqlsrv_fetch_array($select, SQLSRV_FETCH_ASSOC);

    return $registroUsuario ?? array(false, 'Usuario não encontrado');
  }
  ...
  function avaliar($dadosPost, $usuario, $titulo){
    ...
    $insert = sqlsrv_query($this->conn, $sql, $registrosUsuario);//Executa a consulta, comparando com o email passado
    //Verificando se o insert deu certo
    if(!($insert))
      return array(false, "Houve um erro ao postar o comentario, tente novamente.");//Se NÃO der certo, retorna false e não executa o que está abaixo
  ...
  function apagarComentario($idComentario, $usuario, $idtitulo){
    ...
    $sql = "DELETE FROM opniao 
            WHERE id = ? 
              ...
              and usuario = ?";
    //Passando os valores para executar o sql
    $registrosUsuario = array($idComentario, $idtitulo, $idUsuarioComentario);
  ...
  function editarComentario($dadosPost){
    ...
    $sql = "SELECT o.id, titulo, usuario
            ...
            WHERE o.id = ?
              ...
              and t.nome = ?";
    //Passando os valores para executar o sql
    $registrosUsuario = array($idComentario, $idUsuario, $nomeTitulo);
    $select = sqlsrv_query($this->conn, $sql, $registrosUsuario);
    $registroOpniao = sqlsrv_fetch_array( $select, SQLSRV_FETCH_ASSOC);
    ...
    */
    ...
    $update = sqlsrv_query($this->conn, $sql, $registrosUsuario);
    //Verificando se o update deu certo
    if(!($update))
      return array(false, "Houve um erro ao editar o comentario, tente novamente.");//Se NÃO der certo, retorna false e não executa o que está abaixo
  ...
  function mostrarComentarios($idTitulo, $pagina){
    ...
    if($select)
      while($registro = sqlsrv_fetch_array( $select, SQLSRV_FETCH_ASSOC))
        $comentarios[$registro['id']] = $registro;
