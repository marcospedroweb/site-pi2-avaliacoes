...
function mostrarAviso(mostrar, alert) {
  ...
  if (mostrar) {
    ...
    if (!(alert.classList.contains('alert-hidden')) && alert === alertWarning) {
      ...
      alert.classList.remove('alert-danger');
      alert.classList.add('alert-warning');
    } else if (!(alert.classList.contains('alert-hidden')) && alert === alertSuccess) {
      alert.classList.toggle('alert-hidden');
      setTimeout(function () {
        alert.classList.toggle('d-none');
      }, 200);
...
btnFecharAviso.forEach((el) => {
  el.addEventListener('click', () => {
    if (el.getAttribute('id') === 'btn-erro')
      mostrarAviso(false, 'erro');
    else
      mostrarAviso(false, 'sucesso');
  })
});

if (variaveisPHP['sucesso'] === '1')
  mostrarAviso(true, 'sucesso');
